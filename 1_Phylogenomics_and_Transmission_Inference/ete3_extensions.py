...
def parse_nexus_state(tre, state_type="mutation") :
    states = {}
    for node in tre.traverse('postorder'):
        states[node.name] = {}
        for field, state in node.annotations.items():
            if state_type == "state":
                states[node.name][field] = state
            elif state_type == "mutation":
                s0 = node.up.annotations.get(field, '-') if node.up and node.up.annotations else '-'
                states[node.name][field] = [s0, state]
    return states
